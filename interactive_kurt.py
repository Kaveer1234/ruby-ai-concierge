...
if prompt := st.chat_input("Reply to Ruby..."):
    ...
    # Cleaning
    clean = prompt.lower().replace("my name is", "").replace("i am", "").replace("the company is", "").strip().title()
    
    # Global Listeners for Phone/Email
    ...
    # 2. Quote Workflow
        ...
        st.session_state.quote_step = 0 # RESET
        send_to_office({**st.session_state.lead_data, **st.session_state.quote_data}, "FULL QUOTE")
        answer = "Sent! Our sales team will be in touch shortly. Is there anything else I can help with today?"
